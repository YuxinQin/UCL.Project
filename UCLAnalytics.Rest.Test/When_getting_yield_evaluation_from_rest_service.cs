...
{
    [TestFixture]
    public class When_getting_yield_evaluation_from_rest_service : Given_a_rest_service
    {
        ...
        public void Create_server_and_issue_a_get_request()
        {
            using (var host = new InMemoryHost(new OpenRastaConfiguration()))
            {
                ...
                {
                    Uri = new Uri("http://localhost/10000/price?yield=1.5"),
                    HttpMethod = "GET"
                };
        ...
        [Test]
        public void Should_get_result_of_yield_evaluation_by_cusip()
        {                  
            Assert.That(ev.Result, Is.GreaterThan(66));         
            Assert.That(ev.Result, Is.LessThan(67));            
        }         
    }        
}
